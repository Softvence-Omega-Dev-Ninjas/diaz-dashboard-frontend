...
const RightPreviewSection = ({ data }: RightPreviewSectionProps) => {
  ...
  // Format listing name
  const listingName = name || `${buildYear || '2018'} ${make || 'Viking'} ${model || '80 Enclosed Flybridge'}`;
  
  // Format price
  const formattedPrice = price ? `$${parseFloat(price).toLocaleString()}` : '$1,195,000';
  ...
  return (
    <div className="sticky top-6">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm">
        ...
        {/* Listing Name */}
        <h3 className="font-semibold text-base mb-3 line-clamp-2">{listingName}</h3>
        ...
        {/* Price */}
        <p className="text-cyan-600 font-semibold text-lg">Price: {formattedPrice}</p>
      </div>
    </div>
  );
};
